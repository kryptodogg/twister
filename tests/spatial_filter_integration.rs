...
fn test_mouth_region_boundary_negative_15_deg() {
    ...
    let is_mouth_region = test_el >= mouth_el_min && test_el <= mouth_el_max
        && test_az.abs() <= mouth_az_max;

    assert!(is_mouth_region,
        "Elevation {:.1}° and azimuth {:.1}° should match mouth region",
        test_el.to_degrees(), test_az.to_degrees());
}
...
fn test_mouth_region_rejects_above_ear() {
    ...
    let is_mouth_region = above_ear_el >= mouth_el_min && above_ear_el <= mouth_el_max
...
fn test_mouth_region_rejects_wide_azimuth() {
    ...
    let is_mouth_region = mouth_el >= mouth_el_min && mouth_el <= mouth_el_max
        && wide_az.abs() <= mouth_az_max;
...
fn test_mouth_region_boundary_at_minus_30() {
    ...
    let is_mouth_region = at_boundary_el >= mouth_el_min && at_boundary_el <= mouth_el_max
        && frontal_az.abs() <= mouth_az_max;
...
fn test_mouth_region_boundary_at_zero() {
    ...
    let is_mouth_region = at_ear_el >= mouth_el_min && at_ear_el <= mouth_el_max
        && frontal_az.abs() <= mouth_az_max;
...
fn test_mouth_region_boundary_at_30_azimuth() {
    ...
    let is_mouth_region_pos = mouth_el >= mouth_el_min && mouth_el <= mouth_el_max
        && boundary_az_pos.abs() <= mouth_az_max;
    let is_mouth_region_neg = mouth_el >= mouth_el_min && mouth_el <= mouth_el_max
        && boundary_az_neg.abs() <= mouth_az_max;

    assert!(is_mouth_region_pos,
        "Azimuth exactly at +30° should match mouth region");
    assert!(is_mouth_region_neg,
        "Azimuth exactly at -30° should match mouth region");
}
...
fn test_mouth_region_frontal_speech() {
    ...
    let is_mouth_region = voice_freq > 50.0
        && speech_el >= mouth_el_min && speech_el <= mouth_el_max
...
fn test_gain_adjustment_on_mouth_region() {
    ...
    let enhanced_targets: Vec<(f32, f32)> = if mouth_region_detected {
        initial_targets.iter().map(|(freq, _old_gain)| (*freq, 0.95)).collect()
    } else {
    ...
    for &(_freq, gain) in &enhanced_targets {
        assert_eq!(gain, 0.95, "All gains should be maximum (0.95) after mouth-region detection");
    }
}
...
fn test_gain_unchanged_no_mouth_region() {
    ...
    let is_mouth_region = beam_el >= mouth_el_min && beam_el <= mouth_el_max
        && beam_az.abs() <= mouth_az_max;

    assert!(!is_mouth_region, "Coordinates should NOT match mouth region");
    ...
    let final_targets = if is_mouth_region {
        initial_targets.iter().map(|(f, _)| (*f, 0.95)).collect::<Vec<_>>()
    } else {
        initial_targets.clone()
    };

    assert_eq!(final_targets[0].1, 0.5, "First target gain should remain 0.5");
    assert_eq!(final_targets[1].1, 0.3, "Second target gain should remain 0.3");
}
