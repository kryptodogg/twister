...
mod mesh_shader_tests {
    use twister::visualization::mesh_shaders::{MeshShaderPipeline, MeshLodLevel};
    ...
    fn test_lod_level_creation() {
        ...
        assert_eq!(lod_levels.len(), 28, "Must have exactly 28 LOD levels (0-27)");

        // Verify level numbers are sequential
        for (idx, level) in lod_levels.iter().enumerate() {
            assert_eq!(level.level, idx as u32, "LOD level {} should have level number {}", idx, idx);
        }
    }
    ...
    fn test_lod_screen_coverage_thresholds() {
        ...
        // Levels 0-6: 2048px
        for idx in 0..7 {
            assert_eq!(lod_levels[idx].screen_coverage_pixels, 2048,
                "LOD level {} should have 2048px coverage", idx);
        }

        // Levels 7-13: 512px
        for idx in 7..14 {
            assert_eq!(lod_levels[idx].screen_coverage_pixels, 512,
                "LOD level {} should have 512px coverage", idx);
        }

        // Levels 14-20: 128px
        for idx in 14..21 {
            assert_eq!(lod_levels[idx].screen_coverage_pixels, 128,
                "LOD level {} should have 128px coverage", idx);
        }

        // Levels 21-27: 64px
        for idx in 21..28 {
            assert_eq!(lod_levels[idx].screen_coverage_pixels, 64,
                "LOD level {} should have 64px coverage", idx);
        }
    }
    ...
    fn test_lod_vertex_count_decreasing() {
        ...
        for (idx, lod) in lod_levels.iter().enumerate() {
            assert!(
                ...
                idx, lod.vertex_count, prev_count
            );
    ...
    fn test_lod_triangle_count_decreasing() {
        ...
        for (idx, lod) in lod_levels.iter().enumerate() {
            assert!(
                ...
                idx, lod.triangle_count, prev_count
            );
    ...
    fn test_lod_level_0_high_detail() {
        ...
        assert!(level_0.vertex_count >= 512,
            "Level 0 should have at least 512 vertices, got {}",
            level_0.vertex_count);
        assert!(level_0.screen_coverage_pixels >= 2048,
    ...
    fn test_lod_level_27_low_detail() {
        ...
        assert!(level_27.vertex_count <= 128,
            "Level 27 should have at most 128 vertices, got {}",
            level_27.vertex_count);
        assert!(level_27.screen_coverage_pixels <= 64,
        ...
        for (idx, lod) in lod_levels.iter().enumerate() {
            ...
            if lod.vertex_count > 0 {
                ...
                assert!(
                    ...
                    idx, lod.triangle_count, lod.vertex_count, ratio
                );
            }
        }
    }
    ...
    fn test_shader_modules_defined() {
        ...
        let fragment_shader_src = include_str!("../src/visualization/shaders/mesh_lod_fragment.wgsl");

        assert!(!task_shader_src.is_empty(), "Task shader source must not be empty");
        assert!(!mesh_shader_src.is_empty(), "Mesh shader source must not be empty");
        assert!(!fragment_shader_src.is_empty(), "Fragment shader source must not be empty");

        // Verify shader keywords are present
        assert!(task_shader_src.contains("@compute"), "Task shader must have @compute entry");
        assert!(mesh_shader_src.contains("@vertex"), "Mesh shader must have @vertex entry");
        assert!(fragment_shader_src.contains("@fragment"), "Fragment shader must have @fragment entry");
    }
    ...
    fn test_heat_map_tonemap_function_present() {
        ...
        assert!(fragment_shader_src.contains("0.33"), "Heat map should have transition at 0.33");
        assert!(fragment_shader_src.contains("0.67"), "Heat map should have transition at 0.67");
    }
    ...
    fn test_multiple_instances_different_lods() {
        ...
        assert!(selected_lods[2] > selected_lods[1], "300px should select higher LOD than 1000px");
        assert!(selected_lods[3] > selected_lods[2], "100px should select highest LOD");
    }
    ...
    fn test_lod_selection_boundary_conditions() {
        ...
        assert!(lod_at_2048 < 7, "2048px should be in high-detail range (0-6)");
        assert!(lod_at_512 >= 7 && lod_at_512 < 14, "512px should be in medium range (7-13)");
        assert!(lod_at_128 >= 14 && lod_at_128 < 21, "128px should be in low range (14-20)");
    ...
    fn test_lod_metadata_consistency() {
        ...
        for level in &lod_levels {
            ...
            assert!(level.vertex_count > 0, "Level {} has 0 vertices", level.level);
            assert!(level.triangle_count > 0, "Level {} has 0 triangles", level.level);

            // Screen coverage should be in reasonable range
            assert!(level.screen_coverage_pixels >= 64, "Coverage < 64px for level {}", level.level);
            assert!(level.screen_coverage_pixels <= 2048, "Coverage > 2048px for level {}", level.level);
        }
    }
}
