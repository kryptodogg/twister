...
mod trainer_tests {
    ...
    fn test_gradient_clipping_threshold() {
        ...
        assert!(
            grad_norm_decoder < grad_clip_threshold * 1.5,
            "Decoder gradient reasonable"
        );
    }
    ...
    fn test_checkpoint_rotation() {
        ...
        let mut checkpoints = vec![(2.5, 0), (1.8, 1), (1.2, 2), (0.9, 3), (0.8, 4)];
    ...
    fn test_gradient_flow_three_choke_points() {
        ...
        assert!(
            norm_encoder < norm_mamba,
            "Encoder to Mamba gradient growth"
        );
        assert!(
            norm_mamba < norm_decoder,
            "Mamba to Decoder gradient growth"
        );
