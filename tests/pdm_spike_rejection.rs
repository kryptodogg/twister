...
fn test_reject_single_spike() {
    ...
    assert!((output[1] - 0.5).abs() < 0.01, "Spike should be interpolated to 0.5");
...
fn test_reject_multiple_spikes() {
    ...
    assert!((output[1] - 0.3).abs() < 0.01, "First spike should be interpolated");
    assert!((output[4] - 0.2).abs() < 0.01, "Second spike should be interpolated");
}
...
fn test_preserve_high_amplitude_signals() {
    ...
    assert_eq!(spike_count, 0, "Should not detect spike in sustained high amplitude");
...
fn test_spike_at_start() {
    ...
    assert_eq!(spike_count, 0, "Edge spike without proper neighbors should not be counted");
...
fn test_spike_at_end() {
    ...
    assert_eq!(spike_count, 0, "End spike without proper neighbors should not be counted");
...
fn test_consecutive_spikes() {
    ...
    assert_eq!(spike_count, 0, "Consecutive 0.99s don't match isolated spike pattern");
}
...
fn test_edge_case_buffer_sizes() {
    ...
    assert!(output_empty.is_empty(), "Empty buffer should return empty output");

    let (output_single, count_single) = twister::audio::reject_pdm_spikes(&[0.99]);
    assert_eq!(count_single, 0, "Single element can't be spike (no neighbors)");
...
fn test_realistic_pdm_attack_pattern() {
    ...
    assert_eq!(output[10], 0.0, "Spike should be interpolated to neighbor average");
    assert_eq!(output[25], 0.0, "Spike should be interpolated to neighbor average");
}
...
fn test_negative_spikes() {
    ...
    assert!((-output[1] - 0.5).abs() < 0.01, "Negative spike should be interpolated");
}
