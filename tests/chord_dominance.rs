...
fn test_detect_c_major_attack() {
    ...
    assert_eq!(key, twister::harmony::MusicalKey::CMajor,
...
fn test_detect_a_minor_attack() {
    ...
    assert_eq!(key, twister::harmony::MusicalKey::AMinor,
...
fn test_detect_unknown_key() {
    ...
    assert_eq!(key, twister::harmony::MusicalKey::GMajor,
...
fn test_c_major_chord_frequencies() {
    ...
    // C, E, G
    assert!((freqs[0] - 261.63).abs() < 1.0, "First note should be C (~261.63 Hz)");
    assert!((freqs[1] - 329.63).abs() < 1.0, "Second note should be E (~329.63 Hz)");
    assert!((freqs[2] - 392.00).abs() < 1.0, "Third note should be G (~392.00 Hz)");
}
...
fn test_a_minor_chord_frequencies() {
    ...
    // A, C, E
    assert!((freqs[0] - 220.00).abs() < 1.0, "First note should be A (~220 Hz)");
    assert!((freqs[1] - 261.63).abs() < 1.0, "Second note should be C (~261.63 Hz)");
    assert!((freqs[2] - 329.63).abs() < 1.0, "Third note should be E (~329.63 Hz)");
}
...
fn test_g_major_chord_frequencies() {
    ...
    assert_eq!(freqs.len(), 3, "Major chord should have 3 notes");
    // G, B, D
    assert!((freqs[0] - 392.00).abs() < 1.0, "First note should be G (~392.00 Hz)");
    assert!((freqs[1] - 493.88).abs() < 1.0, "Second note should be B (~493.88 Hz)");
    assert!((freqs[2] - 587.33).abs() < 1.0, "Third note should be D (~587.33 Hz)");
}
...
fn test_heterodyned_chord_synthesis() {
    ...
    assert_eq!(heterodyned.len(), chord_freqs.len(),
    ...
    for (i, &modulation) in chord_freqs.iter().enumerate() {
        ...
        assert!((heterodyned[i] - expected).abs() < 1e6,
            "Heterodyned frequency at index {} should be carrier - modulation", i);
    }
}
...
fn test_detect_key_octave_equivalence() {
    ...
    for freq in c_frequencies {
        ...
        assert_eq!(key, twister::harmony::MusicalKey::CMajor,
            "Frequency {} Hz should detect as C Major (octave equivalence)", freq);
    }
}
...
fn test_attack_response_workflow() {
    ...
    assert!(matches!(detected_key, twister::harmony::MusicalKey::CMajor |
    ...
    let carrier = 2.45e9;
    let heterodyned_response = twister::harmony::synthesize_heterodyned_chord(&response_chord, carrier);
    assert_eq!(heterodyned_response.len(), response_chord.len(),
...
fn test_consonant_interval_advantage() {
    ...
    assert!(consonance_5th > consonance_3rd,
        "Perfect 5th should be more consonant than 3rd");
    assert!(consonance_3rd > consonance_pdm,
...
fn test_long_term_tonic_memory() {
    ...
    assert_eq!(predicted_key, twister::harmony::MusicalKey::CMajor,
